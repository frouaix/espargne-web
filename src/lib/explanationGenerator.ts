...
import { formatCurrencyWithCents } from '../utils/format';
...
function generateSummary(
  ...
  if (failureYear !== undefined && failureAge !== undefined) {
    return (
      ...
      `at age ${failureAge}. The portfolio started at $${formatCurrencyWithCents(initialPortfolio)} and ` +
    ...
    return (
      ...
      `years simulated. Starting with $${formatCurrencyWithCents(initialPortfolio)}, the portfolio ${changeDesc} ` +
      `to $${formatCurrencyWithCents(finalPortfolio)} (a ${Math.abs(portfolioChangePct).toFixed(2)}% ` +
...
function analyzeSuccess(
  ...
  if (!success && failureYear !== undefined && failureAge !== undefined) {
    ...
    return (
      ...
      `Ending balance of $${formatCurrencyWithCents(finalPortfolio)} represents ` +
      ...
      `  • Final portfolio value: $${formatCurrencyWithCents(finalPortfolio)}\n` +
...
function analyzeTaxes(
  ...
  return (
    ...
    `Overall Tax Metrics:\n` +
    `  • Total taxes paid: $${formatCurrencyWithCents(totalTaxes)} over ${dataPoints.length} years\n` +
    `  • Average annual tax: $${formatCurrencyWithCents(avgAnnualTax)}\n` +
    `  • Effective tax rate: ${effectiveTaxRate.toFixed(2)}% of gross withdrawals\n\n` +
    `Tax Variation:\n` +
    `  • Lowest tax year: Year ${minTaxYear.year} (age ${minTaxYear.age}) - $${formatCurrencyWithCents(Math.abs(minTaxYear.taxes))}\n` +
    `  • Highest tax year: Year ${maxTaxYear.year} (age ${maxTaxYear.age}) - $${formatCurrencyWithCents(Math.abs(maxTaxYear.taxes))}\n\n` +
    `Withdrawal Sources:\n` +
...
function analyzeWithdrawals(dataPoints: ChartDataPoint[], totalWithdrawals: number): string {
  ...
  return (
    ...
    `Total Withdrawals: $${formatCurrencyWithCents(totalWithdrawals)}\n` +
    `Average Annual: $${formatCurrencyWithCents(avgWithdrawal)}\n\n` +
    `Source Breakdown:\n` +
    `  • Taxable accounts: $${formatCurrencyWithCents(totalTaxable)} (${pctTaxable.toFixed(2)}%)\n` +
    `  • Traditional accounts: $${formatCurrencyWithCents(totalTraditional)} (${pctTraditional.toFixed(2)}%)\n` +
    `  • Roth accounts: $${formatCurrencyWithCents(totalRoth)} (${pctRoth.toFixed(2)}%)\n\n` +
...
function identifyMilestones(dataPoints: ChartDataPoint[]): string[] {
  ...
  if (maxPortfolio.year !== dataPoints[0].year) {
    milestones.push(
      `Portfolio peak: $${formatCurrencyWithCents(maxPortfolio.totalPortfolio)} in year ${maxPortfolio.year} (age ${maxPortfolio.age})`,
    );
  }
  
  if (minPortfolio.totalPortfolio < dataPoints[0].totalPortfolio * 0.8) {
    milestones.push(
      `Portfolio low point: $${formatCurrencyWithCents(minPortfolio.totalPortfolio)} in year ${minPortfolio.year} (age ${minPortfolio.age})`,
    );
  }
  ...
  if (ssaYears.length > 0) {
    ...
    milestones.push(
      ...
      `$${formatCurrencyWithCents(firstSsa.socialSecurity)}/year`,
    );
  }
